...
1. Внешнее приложение вызывает `GET /webhook/contracts/openapi.json`.
   Если caller-у нужны русские описания OpenAPI, он вызывает `GET /webhook/contracts/openapi.json?lang=ru`.
   Если `lang` не указан, workflow возвращает English contract.
