...
function buildSendCode(catalog) {
  return [
    ...
    "const bad = [...invalid(to), ...invalid(cc), ...invalid(bcc), ...(replyTo && !emailRe.test(replyTo) ? [replyTo] : [])];",
    ...
    "return [{ json: { shouldSend: true, statusCode: 200, requestId, templateId, to, cc, bcc, toEmail: to.join(', '), ccEmail: cc.join(', '), bccEmail: bcc.join(', '), replyTo, subject, body: textBody } }];",
  ].join('\n');
}
    ...
    nodes: [
      ...
      },
      {
        parameters: {
          resource: 'email',
          operation: 'send',
          fromEmail: "={{ $env.N8N_MAIL_FROM || 'noreply@local.dev' }}",
